  ...
  <v-row justify="center">
    <v-dialog v-model="dialog" persistent max-width="600px">
      <template v-slot:activator="{ on }">
        <v-btn color="primary" dark v-on="on" class="mt-7" block>
          <h3>จ่าย</h3></v-btn
        >
      </template>
      <v-card>
            ...
            <v-row>
              <v-col cols="12" md="12">
                <v-form>
                  ...
                  <v-text-field type="text" label="Paid" v-model="paidNumber" />
                </v-form>
...
export default {
  ...
  computed: {
    changeMoney() {
      let change = this.paidFloat - this.totalNumber;
      if (change > 0) {
        return change;
      }
      return "";
  ...
  methods: {
    ...
    async onSubmit() {
      ...
      let result = await api.sendTransaction(trans);
      this.dialog = false;
      this.$emit("onResult", {
        status: "ok",
        change: this.changeMoney,
